...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout }) => {
    ...
    if (isLoading) {
        return <div className="flex items-center justify-center min-h-screen bg-gray-800"><LoadingSpinner /></div>;
    }

    return (
        <div className="flex min-h-screen bg-gray-800 text-[var(--secondary-color)]">
                ...
                <div className="p-4 md:p-8">
                        ...
                        {activeView === 'about' && (
                            <div className="bg-gray-900 p-6 rounded-lg shadow-lg">
                                ...
                                <textarea 
                                    ...
                                    rows={10}
                                    className="w-full bg-gray-700 border border-gray-600 rounded-md p-3 text-[var(--secondary-color)] focus:outline-none focus:ring-2 focus:ring-[var(--primary-color)] transition-all"
                            ...
                            <div className="bg-gray-900 p-6 rounded-lg shadow-lg">
                                ...
                                <div className="space-y-4">
                                    {projects.length > 0 ? projects.map(project => (
                                        <div key={project.$id} className="bg-gray-800 p-4 rounded-md flex justify-between items-center">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-400">{project.releaseYear} &bull; {project.projectType}</p>
                                                </div>
                                            </div>
                                        ...
                                        <p className="text-center text-gray-400 py-4">No projects found. Click "Add New Project" to get started.</p>
                                    )}
                                </div>
                            </div>
                        )}
                    ...
                    {(isEditingProject || isCreatingProject) && (
                         <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
                             <div className="bg-gray-900 p-8 rounded-lg shadow-2xl w-full max-w-lg relative text-[var(--secondary-color)] max-h-[90vh] overflow-y-auto">
                                 <button onClick={closeModal} aria-label="Close modal" className="absolute top-4 right-4 text-gray-400 hover:text-white text-2xl">&times;</button>
                                 <h2 className="text-2xl font-bold mb-6">{isEditingProject ? 'Edit Project' : 'Create Project'}</h2>
                                 <form onSubmit={handleProjectSubmit} className="space-y-4">
                                     <input type="text" name="title" value={projectForm.title} onChange={handleProjectFormChange} placeholder="Title" required className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]" />
                                     <textarea name="description" value={projectForm.description} onChange={handleProjectFormChange} placeholder="Short Description" required rows={3} className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]" />
                                     <textarea name="synopsis" value={projectForm.synopsis} onChange={handleProjectFormChange} placeholder="Movie Synopsis" rows={5} className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]" />
                                     <div>
                                        <label htmlFor="posterUrl" className="block text-sm font-medium text-gray-400 mb-1">Poster Image URL</label>
                                        <div className="flex items-center space-x-2">
                                            <input id="posterUrl" type="text" name="posterUrl" value={projectForm.posterUrl} onChange={handleProjectFormChange} placeholder="Enter URL or select from media" required className="flex-grow bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]" />
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select id="projectType" name="projectType" value={projectForm.projectType} onChange={handleProjectFormChange} required className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]">
                                                <option>Movie</option>
                                                <option>Short</option>
                                        ...
                                        <div>
                                            <label htmlFor="releaseYear" className="block text-sm font-medium text-gray-400 mb-1">Release Year</label>
                                            <input type="number" name="releaseYear" value={projectForm.releaseYear} onChange={handleProjectFormChange} placeholder="Release Year" required className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]" />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label htmlFor="status" className="block text-sm font-medium text-gray-400 mb-1">Status</label>
                                            <select id="status" name="status" value={projectForm.status} onChange={handleProjectFormChange} required className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]">
                                        ...
                                        {['Upcoming', 'In Production'].includes(projectForm.status) && (
                                            <div>
                                                ...
                                                <input type="date" id="dueDate" name="dueDate" value={projectForm.dueDate} onChange={handleProjectFormChange} required className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-[var(--secondary-color)]" />
                                            </div>
                                        )}
                                    </div>

                                     <div className="flex justify-end space-x-4 pt-4">
                                         <button type="button" onClick={closeModal} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition-colors flex items-center space-x-2">
                    ...
                    {managingCastProject && (
                         <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
                             <div className="bg-gray-900 p-8 rounded-lg shadow-2xl w-full max-w-2xl relative text-[var(--secondary-color)] max-h-[90vh] flex flex-col">
                                 <button onClick={closeCastModal} aria-label="Close modal" className="absolute top-4 right-4 text-gray-400 hover:text-white text-2xl">&times;</button>
                                 <h2 className="text-2xl font-bold mb-1">Manage Cast & Crew</h2>
                                 <p className="text-gray-400 mb-6 border-b border-gray-700 pb-4">For: <span className="font-semibold text-white">{managingCastProject.title}</span></p>
                                 
                                 <div className="space-y-3 flex-grow overflow-y-auto pr-2">
                                    {currentCast.map((member) => (
                                        <div key={member.id} className="flex items-center space-x-2 p-2 bg-gray-800 rounded-md">
                                            <input type="text" value={member.name} onChange={(e) => handleCastChange(member.id, 'name', e.target.value)} placeholder="Name (e.g., John Doe)" className="w-1/2 bg-gray-700 border border-gray-600 rounded p-2" />
                                            <input type="text" value={member.role} onChange={(e) => handleCastChange(member.id, 'role', e.target.value)} placeholder="Role (e.g., Director)" className="flex-grow bg-gray-700 border border-gray-600 rounded p-2" />
                                    ...
                                    {currentCast.length === 0 && <p className="text-gray-500 text-center py-4">No cast or crew members added yet.</p>}
                                 </div>
                                 ...
                                 <div className="pt-4 mt-4 border-t border-gray-700 flex justify-between items-center">
                                    ...
                                    <div className="flex space-x-4">
                                        <button type="button" onClick={closeCastModal} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition-colors">Cancel</button>
